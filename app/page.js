...
// ── LOCAL BROWSER NOTIFICATIONS (works while app/PWA is open) ─────────────
const NOTIFIED_KEY = "mitchi_notified_v1";
function readNotified() {
  if (typeof window === "undefined") return {};
  return safeJson(localStorage.getItem(NOTIFIED_KEY), {}) || {};
}
...
function wasNotified(key) {
  return !!readNotified()[key];
}
function parseBookingDateTimeMs(b) {
  const [d,m,y] = String(b?.date || "").split("/").map(Number);
  const [hh,mm] = String(b?.time || "").split(":").map(Number);
  if(!d||!m||!y||Number.isNaN(hh)||Number.isNaN(mm)) return 0;
...
function CustomersPage({customers, setCustomers, orders, setOrders, bookings, setBookings, services, toast, onCreateOrder, deleteOrder}) {
  ...
  const save = () => {
    ...
    if(!cleanName) { toast("⚠️ Nhập tên khách!"); return; }
    const cleanForm = {...form, name: cleanName, nick:String(form.nick||"").trim(), phone:String(form.phone||"").trim(), social:String(form.social||"").trim()};
    if(editC) {
      setCustomers(p=>p.map(c=>c.id===editC.id?{...c,...cleanForm}:c));
      toast("✅ Đã cập nhật!");
    } else {
      const nc={id:uid(),...cleanForm,tags:["new"],created:todayStr(),lastOrder:""};
      setCustomers(p=>[nc,...p]);
  ...
  const cleanPhantomCustomersNow = async (singleId = null) => {
    ...
    if (!list.length) { toast("✅ Không phát hiện khách phantom rõ ràng để dọn."); return; }
    const preview = list.slice(0, 10).map(c => c.name || c.nick || c.social || c.id).join(", ");
...
function MessagesPage({toast, replies, setReplies}) {
  ...
  const del = id => {
    markDeleted("replies", id);
    setReplies(p=>p.filter(r=>r.id!==id));
    setDelId(null); setExpanded(null);
    toast("🗑 Đã xoá mẫu!");
  };

  const cleanAutoReplies = () => {
    const auto = replies.filter(isAutoGeneratedReply);
    const deduped = dedupeReplies(replies);
    const dedupeRemoved = replies.length - deduped.length;
    if (!auto.length && !dedupeRemoved) { toast("✅ Không thấy mẫu tự sinh/trùng lặp cần dọn."); return; }
    if (!confirm(`Dọn ${auto.length} mẫu tự sinh và ${dedupeRemoved} mẫu trùng? Các mẫu còn lại sẽ được giữ.`)) return;
    const autoIds = new Set(auto.map(r=>r.id));
    const clean = dedupeReplies(replies.filter(r=>!autoIds.has(r.id)));
    replies.filter(r=>!clean.some(x=>x.id===r.id)).forEach(r=>markDeleted("replies", r.id));
    setReplies(clean);
    toast("🧹 Đã dọn mẫu tự sinh/trùng lặp!");
  };
  ...
  const handleImages = e => {
    ...
    const MAX_IMG_SIZE = 800 * 1024;
    const validFiles = files.filter(file => {
      if (file.size > MAX_IMG_SIZE) { toast(`⚠️ ${file.name} quá lớn. Dùng ảnh dưới 800KB để backup nhẹ hơn.`); return false; }
      return true;
    });
    ...
    <div className="scroll-body">
      ...
      <div className="sec">
        <div style={{display:"flex",justifyContent:"flex-end",gap:8,flexWrap:"wrap",marginBottom:10}}>
          {replies.length>30&&<button className="xs xs-yellow" onClick={cleanAutoReplies}>🧹 Dọn mẫu tự sinh</button>}
      ...
      <input ref={fileRef} type="file" accept="image/*" multiple style={{display:"none"}}
        onChange={e=>{
          ...
          const files=Array.from(e.target.files||[]).filter(file=>{
            if(file.size>800*1024){ toast(`⚠️ ${file.name} quá lớn. Dùng ảnh dưới 800KB.`); return false; }
            return true;
          });
          if(!files.length) return;
...
function NotifForm({shop, setShop, toast, onClose}) {
  const [f,setF] = useState({
    unpaid:shop.notifUnpaid??true,
    booking:shop.notifBooking??true,
    followup:shop.notifFollowup??true,
    browser:shop.notifBrowser??false,
    lead:shop.notifBookingLeadMins??30,
    daily:shop.notifDailyBookingTime||"09:00",
  });
  const [perm,setPerm] = useState(()=>typeof window!=="undefined"&&"Notification" in window?Notification.permission:"unsupported");
  const askPermission = async () => {
    if (typeof window === "undefined" || !("Notification" in window)) { toast("⚠️ Trình duyệt này chưa hỗ trợ notification."); return; }
    try {
      if (Notification.permission === "denied") {
        setPerm("denied");
        toast("🚫 Notification đang bị chặn. Vào Chrome → Cài đặt trang web → Thông báo → Cho phép.");
        return;
      }
      const p = await Notification.requestPermission();
      setPerm(p);
      if (p === "granted") {
        setF(prev=>({...prev,browser:true}));
        toast("✅ Đã cấp quyền notification. Nhớ bấm Lưu cài đặt.");
        setTimeout(()=>showLocalNotification("🐸 Mitchi", "Đã bật thông báo thử thành công.", "mitchi-perm-ok"), 250);
      } else if (p === "denied") {
        toast("🚫 Notification bị chặn. Hãy mở Site settings của Chrome để cho phép lại.");
      } else {
        toast("⚠️ Chưa cấp quyền notification.");
      }
    } catch(e) {
      console.error(e);
      toast("⚠️ Không mở được popup cấp quyền. Hãy mở app bằng HTTPS/Chrome rồi thử lại.");
    }
  };
  const testNotify = () => {
  ...
  return(
    <div className="overlay" onClick={e=>e.target===e.currentTarget&&onClose()}>
      <div className="sheet">
        ...
        <div className={`card ${perm==="denied"?"card-red":perm==="granted"?"card-green":""}`} style={{marginBottom:12}}>
          <div style={{fontWeight:800,marginBottom:6}}>Quyền hiện tại: {perm}</div>
          <div style={{fontSize:12,color:T.muted,lineHeight:1.55}}>
            {perm==="granted"?"Đã cấp quyền. Notification cục bộ hoạt động tốt nhất khi app/PWA đang mở hoặc vừa được mở gần đây.":perm==="denied"?"Chrome đang chặn notification. Vào biểu tượng ổ khoá/Cài đặt trang web → Thông báo → Cho phép, rồi mở lại app.":"Bấm Cấp quyền thông báo để Chrome hỏi quyền."}
          </div>
        </div>
        <div className="card" style={{marginBottom:12}}>
          {[
            {k:"unpaid",  l:"Nhắc đơn chưa thanh toán",    s:"Hiện cảnh báo trên Dashboard"},
            {k:"booking", l:"Nhắc booking sắp tới",         s:"Gửi notification trước lịch hẹn"},
            {k:"followup",l:"Nhắc khách lâu chưa quay lại", s:"Sau 21 ngày không có đơn"},
            {k:"browser", l:"Bật notification trình duyệt", s:"Cần cấp quyền trên Chrome/Safari"},
          ].map((x,i)=>(
            <div key={x.k} style={{display:"flex",alignItems:"center",justifyContent:"space-between",padding:"12px 0",borderBottom:i<3?`1px dashed ${T.border}`:"none"}}>
              <div>
        ...
        <div className="card card-blue" style={{marginBottom:12}}>
          <div className="f">
            <label>Nhắc trước lịch hẹn bao nhiêu phút?</label>
            <select value={f.lead} onChange={e=>setF(p=>({...p,lead:Number(e.target.value)}))}>
              {[5,10,15,30,45,60,120].map(n=><option key={n} value={n}>{n} phút trước</option>)}
            </select>
          </div>
          <div className="f">
            <label>Giờ báo tổng hợp lịch hẹn hôm nay</label>
          ...
          <div style={{display:"flex",gap:8,marginTop:10}}>
            ...
            <button className="xs" onClick={testNotify}>Gửi thử</button>
          </div>
        </div>

        <div style={{display:"flex",gap:8}}>
          <button className="btn btn-primary" style={{flex:2}} onClick={()=>{setShop(p=>({...p,notifUnpaid:f.unpaid,notifBooking:f.booking,notifFollowup:f.followup,notifBrowser:f.browser,notifBookingLeadMins:Number(f.lead)||30,notifDailyBookingTime:f.daily||"09:00"}));onClose();toast("✅ Đã lưu cài đặt thông báo!");}}>Lưu</button>
          <button className="btn btn-ghost" style={{flex:1}} onClick={onClose}>Huỷ</button>
        </div>
      </div>
    </div>
    ...
    <div className="overlay" onClick={e=>e.target===e.currentTarget&&onClose()}>
      <div className="sheet">
        ...
        <div className="card card-green" style={{marginBottom:14,display:"flex",alignItems:"center",gap:12}}>
          <img src="/icon-192.png?v=28" alt="Mitchi app icon" style={{width:56,height:56,borderRadius:16,border:`2px solid ${T.ink}`,boxShadow:`3px 3px 0 ${T.ink}`,background:T.yellow}}/>
          <div>
            <div style={{fontWeight:700,marginBottom:6}}>✅ App đã sẵn sàng cài!</div>
            <div style={{fontSize:13,color:T.muted,lineHeight:1.6}}>Icon app dùng emoji 🐸 hệ thống trên nền xanh #3EAEF4. Link: <strong>mitchi-shop.vercel.app</strong></div>
          </div>
        </div>
...
const DEFAULT_SHOP = {
  ...
  notifFollowup: true,
  notifBrowser:  false,
  notifBookingLeadMins: 30,
  notifDailyBookingTime: "09:00",
  notifSound: false,
};
...
if (typeof window !== "undefined") {
  ...
  manifestLink.href = "/manifest.webmanifest?v=28";
  ...
  appleIcon.href = "/apple-touch-icon.png?v=28";
  ...
  icon32.href = "/favicon-32x32.png?v=28";
  ...
  shortcutIcon.href = "/favicon.ico?v=28";
...
const DELETED_KEY = "mitchi_deleted_v1";
function readDeletedQueue() {
  if (typeof window === "undefined") return {};
  return safeJson(localStorage.getItem(DELETED_KEY), {}) || {};
}
function writeDeletedQueue(deleted) {
  if (typeof window === "undefined") return;
  localStorage.setItem(DELETED_KEY, JSON.stringify(deleted || {}));
}
function markDeleted(table, id) {
  if (!id || typeof window === "undefined") return;
  const deleted = readDeletedQueue();
  deleted[table] = Array.from(new Set([...(deleted[table] || []), id]));
  writeDeletedQueue(deleted);
  clearDirty(table, id);
}
function isDeleted(table, id) {
  if (!id || typeof window === "undefined") return false;
  return (readDeletedQueue()[table] || []).includes(id);
}

const normText = v => String(v || "").trim().toLowerCase().replace(/\s+/g," ");
const isBlankish = v => {
  const x = normText(v);
  return !x || x === "-" || x === "—" || x === "_" || x === "null" || x === "undefined";
};
const isWeakCustomerText = v => {
  const x = normText(v);
  if (isBlankish(x)) return true;
  if (["khach", "khách", "khach moi", "khách mới", "new customer", "customer", "test", "tst", "tes", "abc", "aaa", "zzz", "demo", "phantom"].includes(x)) return true;
...
// Cố tình rộng hơn bản cũ để dọn các khách như "Tst", "Test", "Khách mới", phone "-".
const isClearlyPhantomCustomer = (c, orders = [], bookings = []) => {
  if (!c?.id || c.archived) return false;
  if (hasCustomerActivity(c, orders, bookings)) return false;
  const name = c.name, nick = c.nick, phone = c.phone, social = c.social, notes = c.notes;
  const allBlank = [name,nick,phone,social,notes].every(isBlankish);
  if (allBlank) return true;
  const weakIdentity = isWeakCustomerText(name) && isWeakCustomerText(nick) && isWeakCustomerText(phone) && isWeakCustomerText(social);
  const weakNoNotes = weakIdentity && isBlankish(notes);
  const noPhoneAndWeakName = isWeakCustomerText(phone) && isWeakCustomerText(name) && isBlankish(notes);
  const noPhoneAndWeakSocial = isWeakCustomerText(phone) && isWeakCustomerText(social) && isBlankish(notes) && isWeakCustomerText(name || nick);
  return weakNoNotes || noPhoneAndWeakName || noPhoneAndWeakSocial;
};
const isAutoGeneratedReply = r => {
  const hash = normText(r?.hash);
  const title = normText(r?.title);
  return /#(chao|loingo|nhaclich|xacnhan|feedback|tinhyeu|camon|baogia|thanhtoan|trabai|followup|menu)\d+$/.test(hash) || /#\d+$/.test(title);
};
function dedupeReplies(rows = []) {
  const map = new Map();
  (rows || []).forEach(r => {
    if (!r?.id) return;
    const key = [normText(r.hash), normText(r.title), normText(r.body)].join("|");
    if (!map.has(key)) map.set(key, r);
  });
  return Array.from(map.values());
}

function mergeById(table, cloud = [], local = []) {
  const deletedIds = new Set((readDeletedQueue()[table] || []));
  const dirtyIds = new Set((readDirtyQueue()[table] || []));
  const map = new Map();
  (cloud || []).forEach(x => {
    if (!x?.id || deletedIds.has(x.id)) return;
    map.set(x.id, x);
  });
  (local || []).forEach(x => {
    if (!x?.id || deletedIds.has(x.id)) return;
    // Không cho local cache cũ tự hồi sinh tin mẫu rác nếu cloud đã xoá. Chỉ giữ local nếu đang dirty.
    if (table === "replies" && !map.has(x.id) && !dirtyIds.has(x.id)) return;
    // Không giữ khách hoàn toàn trống để tránh phantom customer.
    if (table === "customers" && !isMeaningfulCustomer(x)) return;
    if (map.has(x.id) || dirtyIds.has(x.id) || table === "orders" || table === "bookings" || table === "customers") {
      map.set(x.id, {...(map.get(x.id) || {}), ...x});
    }
  });
  const out = Array.from(map.values());
  return table === "replies" ? dedupeReplies(out) : out;
}
...
export default function App() {
  ...
  })).filter(x=>x.id && !isDeleted("services", x.id));
  ...
  }).filter(x=>x.id && !isDeleted("orders", x.id));
  ...
  })).filter(x=>x.id && !isDeleted("bookings", x.id));

  const normalizeReplies = (rows) => dedupeReplies((rows || []).filter(Boolean).map(r => ({
    ...r,
    images: safeJson(r.images, []),
  })).filter(x=>x.id && !isDeleted("replies", x.id)));
  ...
  const loadFromDb = async () => {
    ...
    try {
      ...
      const mergedServices = mergeById("services", cloudServices, localServices);
      const mergedCustomers = mergeById("customers", cloudCustomers, localCustomers);
      const mergedOrders = mergeById("orders", cloudOrders, localOrders);
      const mergedBookings = mergeById("bookings", cloudBookings, localBookings);
      const mergedReplies = mergeById("replies", cloudReplies, localReplies);
  ...
  // Local booking notifications. Works while the app/PWA is open or recently active.
  useEffect(() => {
    if (!auth || !dbReady || !shop.notifBrowser || !shop.notifBooking) return;
    if (typeof window === "undefined" || !("Notification" in window)) return;
    const check = () => {
      if (Notification.permission !== "granted") return;
      const now = Date.now();
      const leadMs = (Number(shop.notifBookingLeadMins) || 30) * 60 * 1000;
      bookings
        .filter(b => ["pending","confirmed"].includes(b.status) && b.date && b.time)
        .forEach(b => {
          const when = parseBookingDateTimeMs(b);
          if (!when) return;
          const diff = when - now;
          if (diff >= 0 && diff <= leadMs) {
            const c = customers.find(x=>x.id===b.custId) || b.customerSnapshot || {name:"Khách"};
            const key = `booking-${b.id}-${b.date}-${b.time}-${shop.notifBookingLeadMins||30}`;
            if (!wasNotified(key)) {
              showLocalNotification("🐸 Sắp tới lịch xem bài", `${b.time} · ${c.nick || c.name || "Khách"}`, key);
              markNotified(key);
            }
          }
        });
      const dailyTime = shop.notifDailyBookingTime || "09:00";
      const hhmm = new Date().toLocaleTimeString("vi-VN", {hour:"2-digit", minute:"2-digit", hour12:false});
      const dailyKey = `daily-${todayStr()}-${dailyTime}`;
      if (hhmm === dailyTime && !wasNotified(dailyKey)) {
        const todayCount = bookings.filter(b=>b.date===todayStr() && !["cancel","waiting"].includes(b.status)).length;
        showLocalNotification("🐸 Lịch hẹn hôm nay", todayCount ? `Hôm nay có ${todayCount} booking.` : "Hôm nay chưa có booking.", dailyKey);
  ...
  }, [auth, dbReady, bookings, customers, shop.notifBrowser, shop.notifBooking, shop.notifBookingLeadMins, shop.notifDailyBookingTime]);
  ...
  const setServicesAndSync = (updater) => {
    setServices(prev => {
      ...
      // Delete removed
      prevIds.filter(id=>!nextIds.includes(id)).forEach(id => { markDeleted("services", id); sb.delete("services", id).catch(console.error); });
      return next;
    });
  };
  ...
  const setCustomersAndSync = (updater) => {
    setCustomers(prev => {
      ...
      prevIds.filter(id=>!nextIds.includes(id)).forEach(id => { markDeleted("customers", id); sb.delete("customers", id).catch(e => { console.error("Delete customer:", e); toast("⚠️ Không xoá được khách trên cloud."); }); });
      return next;
    });
  };
  ...
  const setBookingsAndSync = (updater) => {
    setBookings(prev => {
      ...
      prevIds.filter(id=>!nextIds.includes(id)).forEach(id => { markDeleted("bookings", id); sb.delete("bookings", id).catch(e=>{ console.error("Delete booking:", e); toast("⚠️ Không xoá được booking trên cloud."); }); });
      return next;
    });
  };
  ...
  const setRepliesAndSync = (updater) => {
    setReplies(prev => {
      ...
      prevIds.filter(id=>!nextIds.includes(id)).forEach(id => { markDeleted("replies", id); sb.delete("replies", id).catch(e=>{ console.error("Delete reply:", e); toast("⚠️ Không xoá được tin mẫu trên cloud."); }); });
      return next;
    });
  };
